...
namespace Assignment7_V2
{
    class PlayerInventory
    {
        ...
        {
            inventory = new List<Items>(); // instanciates player inventory
            //Cheat();
            AddItem(QuestItems.Rubber_Duck); // and adds 3 items to it
        ...
        public Interactions?[] GetInteractions(Items item, Point? location)
        {
            if (item.TargetLocation == null || location == null) // the targetlocation current menu items target location
                return new Interactions?[] { Interactions.ErrorMessage }; // returns an array indicating to display item's error message

            var interactionsList = new List<Interactions?>(); // a temporary list to simplify readability and returning *
            ...
            return interactionsList.ToArray(); // finally, if there are interactions * returns them as an array
        }
